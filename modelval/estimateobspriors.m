function [obsPrior] = estimateobspriors(valM,oM,colLabels,iF)
%ESTIMATEOBSPRIORS 
...
if nargin < 4
    iF=1; %Inflation factor for priors
end
...
for ii = 1:2
    ...
    obsPrior(proxInd(ii)).std = sqrt(nanmean(vProx)).*iF;
...
for ii=1:nObs
    if ~any(ii==proxInd) %Only make comparisons for satellites
        ...
        obsPrior(ii).std = (sqrt(nanmean(vSat))./sqrt(2)).*iF; %Correct for satellite noise coming from two observers
    end
end
